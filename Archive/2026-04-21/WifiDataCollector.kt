 ...
 * (outside a callback) always returns a [WifiInfo] with SSID/BSSID redacted
 * to `<unknown ssid>` / `02:00:00:00:00:00`, regardless of granted permissions.
 *
 * The only way to receive an unredacted [WifiInfo] on API 31+ is to read
 * `transportInfo` inside a [ConnectivityManager.NetworkCallback] that was
 * registered with [ConnectivityManager.NetworkCallback.FLAG_INCLUDE_LOCATION_INFO].
 * That flag tells the framework the caller holds location permission and wants
 * real values.
 *
 * This class registers a persistent callback on [start] and caches the last
 * good [WifiInfo] it receives. [poll] reads from that cache rather than calling
 * [ConnectivityManager.getNetworkCapabilities] directly.
 */
class WifiDataCollector(private val context: Context) {
    ...
    private val networkCallback = object : ConnectivityManager.NetworkCallback(
        // FLAG_INCLUDE_LOCATION_INFO is the critical flag — without it the
        // WifiInfo delivered to the callback is still redacted on API 31+.
    ...
    /** Register the NetworkCallback. Call once when the ViewModel is created. */
    fun start() {
    ...
    /** Unregister the NetworkCallback. Call when the ViewModel is cleared. */
    fun stop() {
        if (!callbackRegistered) return
        try {
    ...
    suspend fun poll(): PollResult = withContext(Dispatchers.IO) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            Log.d(tag, "  NearbyWifi: ${ContextCompat.checkSelfPermission(context,
        ...
        try {
            ...
            if (ssid.equals("<unknown ssid>", ignoreCase = true) || ssid.isBlank()) {
                ...
                val msg = "SSID still redacted even via NetworkCallback. " +
                        ...
                        "FineLoc=$fineLoc  LocOn=${locationManager.isLocationEnabled}. " +
                        "Try toggling WiFi off and back on, or check that Location " +
                        "permission is set to 'While using the app'."
                Log.w(tag, msg)
                return@withContext PollResult.Error(msg)
            }
            ...
            if (linkProps != null) {
                ...
                // Prefer the default route gateway from the routing table — that
                // is the actual default gateway.  dhcpServerAddress is the DHCP
                // server, which coincides with the gateway on home networks but is
                // a distinct host on enterprise/managed networks.
